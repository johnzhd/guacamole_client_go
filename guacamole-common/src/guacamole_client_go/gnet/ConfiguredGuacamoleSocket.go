 ...
 */
func (opt *ConfiguredGuacamoleSocket) expect(reader gio.GuacamoleReader, opcode string) (instruction gprotocol.GuacamoleInstruction, err exp.ExceptionInterface) {
	...
	if len(instruction.GetOpcode()) == 0 {
		err = exp.GuacamoleServerException.Throw("End of stream while waiting for \"" + opcode + "\".")
		return
	}

	// Ensure instruction has expected opcode
	if instruction.GetOpcode() != opcode {
		err = exp.GuacamoleServerException.Throw("Expected \"" + opcode + "\" instruction but instead received \"" + instruction.GetOpcode() + "\".")
		return
	}
	return
}
...
func NewConfiguredGuacamoleSocket2(socket GuacamoleSocket,
	...
	return NewConfiguredGuacamoleSocket3(socket, config, gprotocol.NewGuacamoleClientInformation())
}
...
*                            initial protocol handshake.
 */
func NewConfiguredGuacamoleSocket3(socket GuacamoleSocket,
	config gprotocol.GuacamoleConfiguration,
	info gprotocol.GuacamoleClientInformation) (one ConfiguredGuacamoleSocket,
	err exp.ExceptionInterface) {
	...
	err = writer.WriteInstruction(gprotocol.NewGuacamoleInstruction("select", selectArg))
	if err != nil {
		return
	}
	...
	err = writer.WriteInstruction(
		gprotocol.NewGuacamoleInstruction(
			"size",
	...
	// Send args
	err = writer.WriteInstruction(gprotocol.NewGuacamoleInstruction("connect", argValueS...))
	if err != nil {
		return
	}
	...
	if len(readyArgs) == 0 {
		err = exp.GuacamoleServerException.Throw("No connection ID received")
		return
	}
...
*         ConfiguredGuacamoleSocket.
 */
func (opt *ConfiguredGuacamoleSocket) GetConfiguration() gprotocol.GuacamoleConfiguration {
	return opt.config
}
...
func (opt *ConfiguredGuacamoleSocket) GetWriter() gio.GuacamoleWriter {
	return opt.socket.GetWriter()
}

// GetReader override GuacamoleSocket.GetReader
func (opt *ConfiguredGuacamoleSocket) GetReader() gio.GuacamoleReader {
	return opt.socket.GetReader()
}

// Close override GuacamoleSocket.Close
func (opt *ConfiguredGuacamoleSocket) Close() (err exp.ExceptionInterface) {
