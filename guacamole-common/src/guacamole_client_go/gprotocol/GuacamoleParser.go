...
func (opt *GuacamoleParser) Append(chunk []byte, offset, length int) (charsParsed int, err exp.ExceptionInterface) {
	charsParsed = 0

	// Do not exceed maximum number of elements
	if len(opt.elements) >= INSTRUCTION_MAX_ELEMENTS && opt.state != COMPLETE {
		opt.state = ERROR
		err = exp.GuacamoleServerException.Throw("Instruction contains too many elements.")
		return
	}
	...
	loop:
		...
		if parsedLength > INSTRUCTION_MAX_LENGTH {
			opt.state = ERROR
			err = exp.GuacamoleServerException.Throw("Instruction exceeds maximum length.")
			return
		}
...
// *         more data can be appended.
// * @throws GuacamoleException If an error occurs while parsing the new data.
func (opt *GuacamoleParser) AppendAll(chunk []byte) (charsParsed int, err exp.ExceptionInterface) {
